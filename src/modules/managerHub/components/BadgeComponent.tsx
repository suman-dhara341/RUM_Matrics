...
import BadgeChartWithIcons from "./BadgeChartWithIcons";
import AnalyticSortButton from "../../../common/AnalyticSortButton";

const BadgeComponent = ({selectedSort, handleSortSelection  }: any) => {
  ...
  const {
    ...
    isFetching: badgeReportIsFetching,
  } = useGetBadgeQuery({
    ORG_ID,
    EMP_ID,
    REPORTS_TO: EMP_ID,
  ...
  if (badgeReportIsLoading || badgeReportIsFetching) {
    return <AnalyticsComponentShimmer />;
  }

  if (badgeReportIsError) {
    return (
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-end items-end" id="analyticsGraph">
        {!badgeReportData?.data || badgeReportData?.data.length === 0 ? (
