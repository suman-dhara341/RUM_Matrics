...
interface ChartProps {
  chartData: { month: string; total_comments: number }[];
  timePeriod: string[];
  feedCommentIsLoading: boolean;
  feedCommentIsError: boolean;
}

const EmployeeFeedCommentCountChart = ({
  chartData,
  timePeriod,
  ...
  const labelFormattedData = timePeriod.map((label) => {
    ...
    return match ? match.total_comments : 0;
  });
  ...
  const options: any = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Total Feed Comment Count Over Time",
        font: {
          size: 16,
          weight: "bold",
        },
...
export default EmployeeFeedCommentCountChart;
