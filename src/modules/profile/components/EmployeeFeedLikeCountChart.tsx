...
interface ChartProps {
  chartData: { month: string; total_likes: number }[];
  timePeriod: string[];
  feedLikeIsLoading: boolean;
  feedLikeIsError: boolean;
}

const EmployeeFeedLikeCountChart = ({
  chartData,
  timePeriod,
  feedLikeIsLoading,
  feedLikeIsError,
  ...
  const labelFormattedData = timePeriod.map((label) => {
    ...
    return match ? match.total_likes : 0;
  });
  ...
  const options: any = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "Total Feed Like Count Over Time",
        font: {
          size: 16,
          weight: "bold",
        },
...
export default EmployeeFeedLikeCountChart;
